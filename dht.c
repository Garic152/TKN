/**
 * dht.c defines a Distributed Hash Table (DHT) implementation using a
 * ring-based structure where each node is responsible for a range of IDs.
 */
...
bool is_peer_equal(struct peer self, struct peer other) {
  return self.id == other.id && self.ip.s_addr == other.ip.s_addr &&
...
static void dht_send(struct dht_message *msg, const struct peer *peer) {
  ...
  if (sendto(dht_socket, msg, sizeof(struct dht_message), 0,
...
static void process_join_o(const struct dht_message *reply) {
...
bool is_in_range(uint start, uint end, uint id) {
...
static void process_join(const struct dht_message *reply) {
  if (is_in_range(predecessor.id, self.id, reply->peer.id)) {
    predecessor = reply->peer;
    dht_notify(self, reply->peer);
  } else {
    dht_join(reply->peer, successor);
 ...
 */
static ssize_t dht_recv(struct dht_message *msg, struct sockaddr *address,
                        socklen_t *address_length) {
  ssize_t result = recvfrom(dht_socket, msg, sizeof(struct dht_message), 0,
 ...
 *
 * Note that this returning false does not imply the passed peer's predecessor
 * is responsible for the ID, this is not generally the case.
 */
static bool is_responsible(dht_id peer_predecessor, dht_id peer, dht_id id) {
  // Gotta store differences explicitly as unsigned since C promotes them to
  // signed otherwise...
  const dht_id distance_peer_predecessor = peer_predecessor - id;
  const dht_id distance_peer = peer - id;
  return (peer_predecessor == peer) ||
         (distance_peer < distance_peer_predecessor);
}
...
struct peer *dht_responsible(dht_id id) {
  ...
  for (size_t i = 0; i < LOOKUP_CACHE_ENTRIES; i += 1) {
    const bool match = is_responsible(lookup_cache[i].predecessor,
                                      lookup_cache[i].peer.id, id);
